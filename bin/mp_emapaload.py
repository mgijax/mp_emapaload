...
def parseMPFile():
    ...
    for e in dom.getElementsByTagName('owl:Class'): # iterate over the NodeList, e is a single Node
        ...
        for n in uberonDescNodes:
            ...
            if str.find(url, 'UBERON_') != -1:
                uberonDescID =  str.split(url, '/')[-1]
    ...
    if DEBUG:
        ...
        for key in keys:
            ...
            uberonDescIds = str.join(', ', rel.id3)
            print('uberonDescIds: %s' % uberonDescIds)
            uberonSomeIds = str.join(', ', rel.id2)
...
def parseUberonFile():
    ...
    if str.strip(lines[0]) != 'format-version: 1.2':
        # sanity #1
...
def findRelationships():
    ...
    for mpId in mpDict:
                ...
                if reportSome:
                    for ubId in uberonSomeList:
                        if ubId in uberonDict:
                            ...
                            if emapaList != []:
                                msg = '%s %s %s %s' % (mpId, mpTerm, ubId, str.join(', ', emapaList))
        ...
        if len(uberonList) > 1:
            ...
            msg = '%s (%s) %s' % (mpId, mpTerm, str.join(', ', termList))
        ...
        for ubId in uberonList:
            ...
            if ubId in uberonAltIdList:
                ...
                if len(emapaList) > 1:
                    ...
                    msg = '%s (%s): %s' % (ubId, ubTerm, str.join(', ', termList))
...
def writeCuratorLog():
    ...
    if mpNotInDatabase:
        ...
        fpLogCur.write('-' * 60 + '\n')
        fpLogCur.write(str.join(CRT, mpNotInDatabase))
    ...
    if emapaNotInDatabase:
        ...
        fpLogCur.write('-' * 60 + '\n')
        fpLogCur.write(str.join(CRT, emapaNotInDatabase))
        fpLogCur.write('%s%s' % (CRT, CRT))
    # #5  
    if mpNoEmapa:
        ...
        fpLogCur.write('-' * 60 + '\n')
        fpLogCur.write(str.join(CRT, mpNoEmapa))
    ...
    if obsAltUberonInMP:
        ...
        fpLogCur.write('-' * 60 + '\n')
        fpLogCur.write(str.join(CRT, obsAltUberonInMP))
    ...
    if obsAltEmapaInUberon:
        ...
        fpLogCur.write('-' * 60 + '\n')
        fpLogCur.write(str.join(CRT, obsAltEmapaInUberon))
    ...
    if oneMpMultiUberon:
        ...
        fpLogCur.write('-' * 60 + '\n')
        fpLogCur.write(str.join(CRT, oneMpMultiUberon))
    ...
    if oneUberonMultiEmapa:
        ...
        fpLogCur.write('-' * 60 + '\n')
        fpLogCur.write(str.join(CRT, oneUberonMultiEmapa))
    ...
    if someValuesFromLost:
        ...
        fpLogCur.write('-' * 60 + '\n')
        fpLogCur.write(str.join(CRT, someValuesFromLost))
